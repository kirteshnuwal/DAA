...
int linearSearch(int arr[], int n, int key, long *comparisons) {
    for (int i = 0; i < n; i++) {
        (*comparisons)++;
        if (arr[i] == key) {
            return i;
...
int main() {
    ...
    for (int i = 0; i < 5; i++) {
        long comparisons = 0;
        clock_t start = clock();
        int index = linearSearch(arr, n, keysToFind[i], &comparisons);
        clock_t end = clock();
    ...
    long comparisons = 0;
    clock_t start = clock();
    int index = linearSearch(arr, n, missingKey, &comparisons);
    clock_t end = clock();
    double timeTaken = ((double)(end - start)) / CLOCKS_PER_SEC * 1000.0;
    ...
    for (int r = 0; r < repeats; r++) {
        linearSearch(arr, n, missingKey, &dummyComparisons);
    }
